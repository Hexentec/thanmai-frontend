...
export default function TestimonialsSlider() {
  ...
  return (
    <section className="ts-section" aria-label="Customer Testimonials" role="region">
      ...
      <div className="ts-slider-wrapper">
        ...
        >
          {list.map(t => (
            <div key={t._id} className="ts-card" role="listitem" aria-label={t.authorName}>
      ...
      <div className="ts-controls">
        <button className="ts-arrow" onClick={prev} disabled={index===0} aria-label="Previous testimonials">‹</button>
        <button className="ts-arrow" onClick={next} disabled={index===maxIndex} aria-label="Next testimonials">›</button>
      </div>
    </section>
  );
}
