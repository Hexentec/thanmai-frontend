...
export default function ShopCategoryPage({ params }) {
  ...
  useEffect(() => {
    api.get('/categories').then(r => {
      setCats(r.data);
      const found = r.data.find(c => c.slug.toLowerCase() === slug.toLowerCase());
      setCat(found);
      if (found) {
  ...
  // Filter products by current category ID before passing to useProductFilter
  const categoryProducts = products.filter(p => p.category && p.category._id === cat._id);
  const filtered = useProductFilter(categoryProducts, filters, sortBy);

  if (!cat) {
    return (
      <p className="shop-loading" style={{ padding: '2rem', textAlign: 'center' }}>
        Category “{slug}” not found. (Check category slug in URL and backend)
      </p>
    );
  }
